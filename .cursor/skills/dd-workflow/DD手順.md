...
**重要**: 作業対象は **案件ルート**（例: `deals/株式会社ABC`）です。新規案件は `dd_logic/template` を `deals/{会社名}/` にコピーして作成し、依頼時に案件ルートを指定して実行します。

**擬似コマンドシステム**: このリポジトリでは、**擬似スラッシュコマンド**で Cursor に作業を依頼できます。
...
各コマンドの詳細は `.cursor/skills/dd-common/SKILL.md`（コマンド一覧・案件ルート）を参照してください。

### このガイドの構成

- **概要**: このファイル（`DD手順.md`）で全体像を把握
- **詳細手順・実行内容**: `.cursor/skills/` の各スキル（dd-copy-classify, dd-summarize 等）の SKILL.md を参照
...
- **全体像・擬似コマンド一覧・システム指示**: `.cursor/skills/dd-common/SKILL.md`
- **DD手順の入口**: `.cursor/skills/dd-workflow/SKILL.md`
...
**目的**: **案件ルート**内の `業務整理用/` フォルダのファイルを、同案件の `dd materials/` の各セクションに適切にコピーして分類します。

**コマンド**: `/dd:copy`（対象は指定した案件ルート）  
**詳細**: `.cursor/skills/dd-copy-classify/SKILL.md`

**実行**: **`program/run_copy.py`** を案件ルートを引数で実行する（例: `python program/run_copy.py deals/株式会社ABC`）。実行過程で **PDF→PNG**（`program/convert_pdf_to_png.py`）・**XLSX→CSV**（`program/convert_xlsx_to_csv.py`）の変換が行われ、変換・コピーした資料は **案件ルートの `dd materials/` のそれぞれのフォルダ**（対応表のコピー先。`dd_logic/template/dd materials` と同じ構成）に保存される。詳細は `program/README.md` の「変換結果の保存先」を参照。
...
**注意**: 元のファイルは案件ルート内の `業務整理用/` フォルダに残しておきます（バックアップとして）。

---

### ステップ2.5: 各フォルダ内のファイルサマリの作成

**目的**: 案件ルート内でコピーしたファイルのうち、Markdownファイル以外（PDF、Excel、Word、画像等）のサマリを、各フォルダ内の `{フォルダ名}サマリ.md` に記載します。

**コマンド**: `/dd:summarize`  
**詳細**: `.cursor/skills/dd-summarize/SKILL.md`
...
**コマンド**: `/dd:update_basics`  
**詳細**: `.cursor/skills/dd-update-basics/SKILL.md`（事業概要・投資条件の両方）

#### 3.1 事業概要の更新

- **対象ファイル**: 案件ルート内の `dd materials/business/事業概要.md`
- **参照資料**: ピッチ資料、事業計画書、営業資料等
- **記載内容**: 事業内容、サービス、ターゲット市場、事業モデル、過去の事業の変化・進化

#### 3.2 投資条件の更新

- **対象ファイル**: 案件ルート内の `dd materials/round table/投資条件概要.md`
...
**コマンド**: `/dd:questions`  
**詳細**: `.cursor/skills/dd-hearing-list/SKILL.md`

- **対象ファイル**: 案件ルート内の `review_and_hypothesis/dd question/ヒアリングリスト.md`
- **作業内容**: 
  - 既に把握している情報を反映
  - 不足している情報を特定
  - 優先度の高い質問項目を明確化
...
**目的**: 案件ルート内の `dd materials/` と `research/` の資料を読み込み、評価結果を同案件の `review_and_hypothesis/` に整理します。

**コマンド**: `/dd:review`（領域別の目的・対象・出力は同一スキル内に記載）  
**詳細**: `.cursor/skills/dd-initial-review/SKILL.md`
...
**目的**: 初期レビューを踏まえ、投資テーゼと複数シナリオ（楽観的・現実的・悲観的）を構築します。共通の「どう考えるか」は **dd_logic/investment_methodology/** を参照しつつ、案件ごとの結論を記述します。

**コマンド**: `/dd:thesis`  
**詳細**: `.cursor/skills/dd-investment-thesis/SKILL.md`
...
- review_and_hypothesis/senario/investment_theory.md: 投資テーゼ・成長シナリオ（共通思考法は dd_logic/investment_methodology 参照）
...
**コマンド**: `/dd:research`  
**詳細**: `.cursor/skills/dd-research/SKILL.md`
...
**コマンド**: `/dd:resource`（Web記事・Notion情報の両方）  
**詳細**: `.cursor/skills/dd-resources/SKILL.md`

#### 8.1 Web記事の保存

- **保存先**: 案件ルート内の `dd materials/web resource/Web/`
- **記載内容**: 記事のURL、公開日、主要な内容、投資判断への関連性

#### 8.2 Notion情報の保存

- **保存先**: 案件ルート内の `dd materials/web resource/Notion/`
...
**コマンド**: `/dd:wrap`  
**詳細**: `.cursor/skills/dd-completion/SKILL.md`（チェックリスト・週次・月次含む）

#### 完了確認チェックリスト（対象は指定した案件ルート）
...
**詳細**: `.cursor/skills/dd-init/SKILL.md`

このコマンドは、指定した案件ルートに対してステップ2からステップ4までを一括で実行します。
...
### ガイド・コマンド・チェックリスト
- **擬似コマンド一覧・システム指示・品質基準**: `.cursor/skills/dd-common/SKILL.md`
- **DD手順の入口**: `.cursor/skills/dd-workflow/SKILL.md`
- **チェックリスト（DD完了・週次・月次）**: `.cursor/skills/dd-completion/SKILL.md`

### 各ステップの詳細（スキル）
- `.cursor/skills/dd-copy-classify/SKILL.md` — 資料分類・配置
- `.cursor/skills/dd-summarize/SKILL.md` — サマリ作成
- `.cursor/skills/dd-update-basics/SKILL.md` — 基本情報更新
- `.cursor/skills/dd-hearing-list/SKILL.md` — ヒアリングリスト更新
- `.cursor/skills/dd-initial-review/SKILL.md` — 初期レビュー
- `.cursor/skills/dd-investment-thesis/SKILL.md` — 投資仮説作成
- `.cursor/skills/dd-research/SKILL.md` — リサーチ
- `.cursor/skills/dd-resources/SKILL.md` — 参考資料整理
- `.cursor/skills/dd-completion/SKILL.md` — 完了報告
- `.cursor/skills/dd-ic-slides/SKILL.md` — 投資委員会資料骨格
